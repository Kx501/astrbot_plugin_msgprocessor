...
function ModuleConfigFields({
  ...
  const c = step.config;
  const set = (patch: Record<string, unknown>) => onChange({ ...step, config: { ...step.config, ...patch } });
      ...
      return (
        <div className="pipeline-config-grid">
          ...
          <label className="field-inline-check field-inline-check--align-input">
            <input type="checkbox" checked={Boolean(c.whole_from_empty)} onChange={(e) => set({ whole_from_empty: e.target.checked })} />
            <span>{UI.cfgWholeFromEmpty}</span>
          </label>
          <label className="field-inline-check field-inline-check--align-input">
            <input type="checkbox" checked={Boolean(c.regex)} onChange={(e) => set({ regex: e.target.checked })} />
            <span>{UI.cfgReplaceRegex}</span>
          </label>
          <label className="field-stack">
            <span className="label-text">{UI.cfgRegexFlags}</span>
            <input value={String(c.regex_flags ?? "")} onChange={(e) => set({ regex_flags: e.target.value })} />
          </label>
        </div>
      );
    case "translate_llm":
  ...
  return (
    <div className="pipeline-editor">
      <div className="pipeline-toolbar">
        <button type="button" className="btn" onClick={() => onChange([...pipeline, { _key: newKey(), id: "noop", config: {} }])}>
          {UI.addModule}
        </button>
